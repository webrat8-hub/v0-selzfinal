...
import { Shield, Bug, LayoutDashboard, Settings, Loader2, Music, RotateCcw, ChevronLeft, ChevronRight, Volume2, VolumeX, Zap, EyeOff, Copy, CheckCircle2, AlertTriangle, ExternalLink, Lock } from "lucide-react"
  ...
  return (
    <div className={`relative min-h-screen bg-black text-white font-sans overflow-hidden transition-opacity duration-500 ${isStealth ? 'opacity-30' : 'opacity-100'}`}>
      ...
      <div className="fixed inset-0 z-0">
        ...
        {/* Rain Overlay Layer */}
        <div className="absolute inset-0 pointer-events-none rain-overlay opacity-30"></div>
      </div>
      <audio ref={bgMusicRef} src="/audio.mp3" loop autoPlay={isMusicOn} />

      {/* --- OVERLAY: LOGIN GAGAL (GLITCH & SHAKE) --- */}
      {showErrorOverlay && (
      ...
      {!isLoggedIn ? (
        <div className="relative z-10 flex flex-col items-center justify-center min-h-screen p-6 animate-in slide-in-from-top-10 duration-700">
          ...
          <div className="w-full max-w-sm bg-white/5 border border-white/10 backdrop-blur-3xl rounded-[3rem] p-10 shadow-2xl border-t-white/20">
            <div className="bg-black/40 p-4 rounded-2xl mb-6 border border-white/5 text-center font-black text-cyan-400 italic text-sm">OPERATOR: SELZ</div>
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="bg-gradient-to-br from-white/10 to-transparent border border-white/10 rounded-[3rem] p-8 mb-8 text-center backdrop-blur-md relative shadow-2xl">
                <div className="flex justify-between items-center absolute inset-x-4 top-1/2 -translate-y-1/2">
                   <button onClick={() => setActiveNav(prev => (prev - 1 + BUG_TYPES.length) % BUG_TYPES.length)} className="p-3 bg-black/40 rounded-full hover:bg-cyan-500 transition-all"><ChevronLeft size={24}/></button>
                   <button onClick={() => setActiveNav(prev => (prev + 1) % BUG_TYPES.length)} className="p-3 bg-black/40 rounded-full hover:bg-cyan-500 transition-all"><ChevronRight size={24}/></button>
                </div>
              ...
              <div className="relative mb-6">
                <input value={targetNumber} onChange={(e) => setTargetNumber(e.target.value)} className="w-full bg-black/60 border border-white/10 p-6 rounded-3xl text-center font-black italic text-xl text-cyan-400 pr-16 focus:border-cyan-500 outline-none transition-all shadow-inner" placeholder="628XXXXXXXX" />
              ...
              <button onClick={handleSendBug} className="w-full py-6 bg-gradient-to-r from-pink-600 via-red-600 to-orange-600 rounded-[2.5rem] font-black uppercase italic text-sm tracking-[0.2em] shadow-xl active:scale-95 transition-all text-white">KIRIM BUG</button>
            </div>
          ) : (
            /* --- SETTINGS VIEW --- */
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              <h2 className="text-3xl font-black italic uppercase mb-10 border-b border-white/10 pb-4 text-cyan-400">Settings Control</h2>
      ...
      <style jsx global>{`
        ...
        @keyframes shake {
          ...
          20% { transform: translate(-7px, 1px) rotate(1deg); }
